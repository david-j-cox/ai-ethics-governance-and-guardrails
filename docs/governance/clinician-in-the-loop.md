...
- **Purpose-built beats general-purpose, and input quality matters.** In a [pilot of a fine-tuned small model for pediatric occupational-therapy SOAP notes](https://ai.jmir.org/2026/1/e73274) (compared against a general assistant), the purpose-built model produced notes rated higher on clarity, completeness, and organization, but measured time savings appeared only when therapists wrote sparse scratch notes and disappeared when they reverted to detailed input. A scribe is not a free efficiency gain; the surrounding workflow determines whether it helps.
